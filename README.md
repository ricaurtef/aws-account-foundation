...
│                                                         │
│  ┌────────────────┐  ┌─────────────────────────────┐   │
│  │ IAM Identity   │  │ OIDC Federation              │   │
│  │ Center (SSO)   │  │                               │   │
│  │                │  │ Provider: GitHub Actions       │   │
│  │ (imported)     │  │ Trust: ricaurtef/*:main        │   │
│  └────────────────┘  │ Role: site deploy permissions  │   │
│                      └─────────────────────────────┘   │
│                                                         │
│  ┌─────────────────────────────────────────────────┐   │
│  │ S3: ricaurtef-terraform-state                    │   │
...
├── env/
│   └── web-prod.tfvars    # Environment-specific values
...
terraform apply -var-file=env/web-prod.tfvars
```
...
PR opened/updated                              Merge to main
       ...
       ▼                                               ▼
┌────────────────────────────────┐    ┌────────────────────────────────┐
│ check │ validate │ tflint │   │    │ check │ validate │ tflint │   │
│ security (parallel)           │    │ security (parallel)           │
└──────────────┬────────────────┘    └──────────────┬────────────────┘
               │                                    │
               ▼                                    ▼
        ┌────────────┐                    ┌───────────────────┐
        │    plan     │                   │ deploy (production)│
        │ (review)    │                   │ plan → ⏸️ approve   │
        └────────────┘                    │ → apply            │
                                          └─────────┬─────────┘
                                                    │
                                                    ▼
...
- Environment values from `env/web-prod.tfvars` via `-var-file`.
